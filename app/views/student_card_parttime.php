<section id="izvanredni" class="content-section">
<div class="container">

	<h3>Upisni list: {{ @card_owner.email }}</h3>

	<form id="pdf" method="POST" action="{{ @BASE.@ALIASES.makePDFCard }}">
...
<div class="col-lg-6">
	<div class="courses">
		...
		<loop from="{{ @sem=1 }}" to="{{ @sem < 9 }}" step="{{ @sem++ }}">
			...
			<div id="{{ @sem }}" class="collapse semester">
				<repeat group="{{ @courses }}" value="{{ @course }}">
					<check if="{{ @course.sem_izvanredni == @sem }}">
	...
	<loop from="{{ @sem=1 }}" to="{{ @sem < 9 }}" step="{{ @sem++ }}">
		<div id="{{ 'sem_'.@sem }}" class="semester">
			...
			<repeat group="{{ @card }}" value="{{ @card_course }}">
				<check if="{{ @card_course.sem_izvanredni == @sem }}">
